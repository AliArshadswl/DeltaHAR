"""
adl_patchhar_v2_best.py
========================
PatchHAR v2  --  ADL  --  Best configuration from ablation sweep.

Ablation winner  (HSMM-F1=0.7945 | kappa=0.7765 | MCC=0.8267):
    D_MODEL   = 768   (unchanged)
    N_HEADS   = 12    (unchanged)
    N_LAYERS  = 8     ← reduced from 12
    N_EXPERTS = 4     (unchanged)

All 10 contributions remain ON (C3 disabled -- no timestamps in ADL).
Leave-One-Group-Out (participant-level) cross-validation.
"""
...
# =============================================================================
# 2.  Configuration  --  BEST from ablation
# =============================================================================
class Config:
    # Data
    ...
    # ── BEST MODEL CONFIG (ablation winner: N_LAYERS=8) ──────────────────
    D_MODEL     = 768
    N_HEADS     = 12
    N_LAYERS    = 8     # ← changed from 12
    N_EXPERTS   = 4
    # ─────────────────────────────────────────────────────────────────────
    DROPOUT     = 0.1
    SD_DROP_MAX = 0.10
...
def _load_local(data_dir: Path):
    ...
    if X.ndim != 3 or X.shape[1] != cfg.WINDOW_SIZE or X.shape[2] != cfg.CHANNELS:
        raise ValueError(f"Expected X(N,{cfg.WINDOW_SIZE},{cfg.CHANNELS}), got {X.shape}")
...
def _load_remote():
    ...
    arrays = {}
    for fname in ("X.npy", "Y.npy", "pid.npy"):
        url = f"{cfg.REMOTE_BASE}/{fname}"; print(f"  Downloading {fname} ...")
        r = requests.get(url); r.raise_for_status()
        arrays[fname[:-4]] = np.load(BytesIO(r.content), allow_pickle=True)
    X = arrays["X"].astype(np.float32); Y = arrays["Y"].astype(str); pid = arrays["pid"].astype(str)
    print(f"  Downloaded ADL: X{X.shape}"); return X, Y, pid
...
def _magnitude_scale(sig): return sig * np.random.uniform(0.8, 1.2, (1, sig.shape[1]))
...
def freq_augment(sig):
    return random.choice([_bandpass_jitter, _axis_permute, _magnitude_scale, _time_warp])(sig)
...
class ADLDataset(Dataset):
    def __init__(self, entries, pid_to_idx, is_train=False):
        self.entries    = entries; self.pid_to_idx = pid_to_idx; self.is_train = is_train
        self.patch_lens = cfg.PATCH_LENS_MULTI if CC.C4_MULTISCALE_PATCHING else [cfg.PATCH_LEN]

    def __len__(self): return len(self.entries)

    @staticmethod
    def _normalise(win):
        mu = np.nanmean(win, axis=0, keepdims=True); sd = np.nanstd(win, axis=0, keepdims=True)
    ...
    def __getitem__(self, idx):
        ...
        patches_list = [torch.from_numpy(self._make_patches(seg, pl)) for pl in self.patch_lens]
        return (patches_list, torch.tensor(lab, dtype=torch.long),
...
def _collate(batch):
    ...
    return ([torch.stack([b[s] for b in patches_lists]) for s in range(n_scales)],
            torch.stack(labels), torch.stack(pids), torch.stack(seqs), torch.stack(raws))
...
def class_weights_from_entries(entries, K):
    ...
    w = torch.tensor(counts.max() / np.clip(counts, 1, None), dtype=torch.float32)
    return w / w.sum() * K
...
def sample_weights_from_entries(entries, K):
    ...
    return torch.tensor([inv[lab] for (_, _, lab, _) in entries], dtype=torch.float32)
...
class GatedDeltaNet(nn.Module):
    def __init__(self, d, dropout=0.1):
        ...
        for name in ("q","k","v"):
            setattr(self, f"{name}_lin",  nn.Linear(d, d))
            setattr(self, f"{name}_conv", nn.Conv1d(d, d, 3, padding=1, groups=d))
        self.act = nn.Sigmoid(); self.alpha = nn.Linear(d, d); self.beta = nn.Linear(d, d)
        self.post_norm = ZCRMSNorm(d); self.post = nn.Linear(d, d)
        self.silu = nn.SiLU(); self.gate = nn.Sigmoid(); self.drop = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        q, k  = self._l2(q), self._l2(k); delta = q * (k * v)
...
class SoftMoE(nn.Module):
    def __init__(self, d, hidden, n_experts=4, dropout=0.1):
        ...
        self.experts = nn.ModuleList([
            nn.Sequential(nn.Linear(d, hidden), nn.SiLU(), nn.Dropout(dropout), nn.Linear(hidden, d))
...
class GatedAttention(nn.Module):
    def __init__(self, d, n_heads=2, dropout=0.1):
        ...
        self.out  = nn.Linear(d, d); self.gate = nn.Linear(d, d); self.drop = nn.Dropout(dropout)
    ...
    def forward(self, x, freqs):
        ...
        raw   = (q @ k.transpose(-2,-1)) / math.sqrt(self.dh)
        attn  = torch.softmax(raw, dim=-1)
        self.last_attn = attn.detach()          # (B, H, N, N) — for rollout
        y     = self.out(self.drop(attn @ v).transpose(1,2).contiguous().reshape(B, N, D))
...
class ReconHead(nn.Module):
    def __init__(self, d, n_patches, channels):
        super().__init__()
        self.mlp = nn.Sequential(nn.Linear(d, 2*d), nn.ReLU(), nn.Linear(2*d, n_patches*channels))
    def forward(self, z): return self.mlp(z)


# =============================================================================
# 9.  PatchHAR v2  (best config)
# =============================================================================
class PatchHARv2(nn.Module):
    def __init__(self, num_classes: int):
        ...
        self.patch_embeds = nn.ModuleList([EmbedCls(pl, cfg.CHANNELS, d) for pl in self.patch_lens])

        if CC.C4_MULTISCALE_PATCHING and len(self.patch_lens) > 1:
            self.scale_fusion = nn.Linear(d * len(self.patch_lens), d)
        else: self.scale_fusion = None
        ...
        if CC.C8_STOCHASTIC_DEPTH:
            ...
            self.delta_layers = nn.ModuleList([StochasticDepth(l, p) for l, p in zip(raw_layers, survs)])
        else: self.delta_layers = nn.ModuleList(raw_layers)

        if CC.C2_CALANET_SKIP_AGG: self.skip_agg = SkipAggregation(d, cfg.N_LAYERS)
        ...
        if CC.C6_LABEL_SMOOTH_TEMP: self.log_tau = nn.Parameter(torch.zeros(1))
        if CC.C10_RECON_AUX_GRAD_SURGERY: self.recon_head = ReconHead(d, NP, cfg.CHANNELS)
        if CC.C7_PROTOTYPE_MEMORY:
            self.register_buffer("prototypes", torch.zeros(num_classes, d)); self.proto_filled = False

        self.num_classes = num_classes
        self.head = nn.Sequential(
            nn.Dropout(0.2), nn.Linear(d, d//2), nn.ReLU(), nn.Dropout(0.1), nn.Linear(d//2, num_classes))
    ...
    def _embed(self, patches_list):
        ...
        if CC.C4_MULTISCALE_PATCHING and len(patches_list) > 1:
            ...
            for embed, p in zip(self.patch_embeds, patches_list):
                e = embed(p)
                e = F.interpolate(e.permute(0,2,1), size=NP, mode="linear", align_corners=False).permute(0,2,1)
                embs.append(e)
    ...
    def _backbone(self, x):
        ...
        for layer in self.delta_layers: x = layer(x); hiddens.append(x)
        if CC.C2_CALANET_SKIP_AGG: x = x + self.skip_agg(hiddens)
        x = x + self.moe1(x); x = self.attn(x, self.freqs); x = x + self.moe2(x)
        return x.mean(dim=1), hiddens

    def forward(self, patches_list, return_embedding=False):
        x = self._embed(patches_list); z, _ = self._backbone(x)
        if return_embedding: return z
        recon = None
        if CC.C10_RECON_AUX_GRAD_SURGERY and self.training: recon = self.recon_head(z)
        logits = self.head(z)
        if CC.C6_LABEL_SMOOTH_TEMP: logits = logits / torch.exp(self.log_tau).clamp(0.5, 2.0)
        if CC.C7_PROTOTYPE_MEMORY and not self.training and self.proto_filled:
            z_n = F.normalize(z, dim=-1); pr_n = F.normalize(self.prototypes, dim=-1)
            logits = (1 - cfg.PROTO_ALPHA) * logits + cfg.PROTO_ALPHA * (z_n @ pr_n.T)
        return logits, recon
    ...
    def update_prototypes(self, embs, labels):
        ...
        for k in range(self.num_classes):
            ...
            self.prototypes[k] = m * self.prototypes[k] + (1-m) * mean if self.proto_filled else mean
...
def recon_loss_fn(recon, raw_segs):
    ...
    target  = raw_segs[:, :NP*PL, :].reshape(B, NP, PL, C).mean(dim=2).reshape(B, NP*C)
...
def tc_loss(logits):
    ...
    return 0.5 * (F.kl_div(q.log(), p, reduction="batchmean") + F.kl_div(p.log(), q, reduction="batchmean"))


def manifold_mixup(z, labels, alpha=0.2):
    if alpha <= 0: return z, labels, labels, 1.0
    lam = float(np.random.beta(alpha, alpha)); idx = torch.randperm(z.size(0), device=z.device)
...
def fit_temperature(logits, labels):
    ts  = TemperatureScaling().to(device); y = torch.from_numpy(labels).to(device)
    x   = logits.to(device).detach(); ce = nn.CrossEntropyLoss()
    opt = optim.LBFGS(ts.parameters(), lr=cfg.TEMP_LR, max_iter=cfg.TEMP_MAX_ITERS,
                      line_search_fn="strong_wolfe")
    def closure():
        opt.zero_grad(set_to_none=True); loss = ce(ts(x), y); loss.backward(); return loss
    opt.step(closure)
    with torch.no_grad(): nll = ce(ts(x), y).item()
    print(f"    T={ts.temperature():.3f} | val NLL={nll:.4f}"); return ts
...
def collect_logits(model, loader):
    ...
    with torch.no_grad():
        for pl, lbl, pid_idx, seq, _ in loader:
            pl = [p.to(device).float() for p in pl]; logits, _ = model(pl)
...
def viterbi(E_log, log_pi, log_A, lam=0.75):
    ...
    for t in range(1, T):
        prev  = dp[t-1, :, None] + log_A - penalty; bp[t] = np.argmax(prev, axis=0)
...
def tune_lambda(ts_model, val_logits, val_true, val_seqs, train_entries, K):
    ...
    log_pi = np.log(np.clip(pi, cfg.HMM_MIN_PROB, 1.0)); log_A = np.log(np.clip(A, cfg.HMM_MIN_PROB, 1.0))
    with torch.no_grad(): probs = ts_model(val_logits.to(device)).softmax(1).cpu().numpy()
    order = np.argsort(val_seqs); E_log = np.log(np.clip(probs[order], cfg.HMM_MIN_PROB, 1.0))
    best_lam, best_f1 = 0.75, -1.0
    for lam in [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5]:
        path = viterbi(E_log, log_pi, log_A, lam); pred = np.empty_like(path); pred[order] = path
...
def multiclass_mcc(yt, yp):
    ...
    if n == 0: return 0.0
    s, t, p = np.trace(cm), cm.sum(1), cm.sum(0); num = s*n - np.sum(t*p)
...
def train_one_fold(model, train_loader, val_loader, class_w, epochs, patience):
    total  = epochs * max(1, len(train_loader)); warmup = int(cfg.WARMUP_FRAC * total)
    ...
    ema = EMA(model, decay=cfg.EMA_DECAY); best_score = -1e9; best_state = None; pat_ctr = 0

    for epoch in range(epochs):
        model.train()
        for batch in train_loader:
            pl, labels, _, _, raw_segs = batch
            pl = [p.to(device).float() for p in pl]; labels = labels.to(device).view(-1)
            raw_segs = raw_segs.to(device).float(); opt.zero_grad(set_to_none=True)

            if CC.C9_MANIFOLD_MIXUP:
                with amp_ctx(): x = model._embed(pl); z, _ = model._backbone(x)
                z_mix, la, lb, lam = manifold_mixup(z, labels, cfg.MIXUP_ALPHA)
                with amp_ctx():
                    logits = model.head(z_mix)
                    if CC.C6_LABEL_SMOOTH_TEMP: logits = logits / torch.exp(model.log_tau).clamp(0.5, 2.0)
                    ...
                    if CC.C10_RECON_AUX_GRAD_SURGERY:
                        loss = loss + cfg.RECON_LAMBDA * recon_loss_fn(model.recon_head(z.detach()), raw_segs)
            else:
                with amp_ctx():
            ...
            if torch.isfinite(loss): nn.utils.clip_grad_norm_(model.parameters(), cfg.MAX_GRAD_NORM); scaler.step(opt)
        ...
        ema.copy_to(model); model.eval(); vp, vt, embs_v, labs_v = [], [], [], []
        with torch.no_grad():
            for pl, lbl, _, _, _ in val_loader:
        ...
        f1  = f1_score(vt, vp, average="macro", zero_division=0); kap = cohen_kappa(vt, vp)
        print(f"    Ep {epoch+1:03d}/{epochs} | F1={f1:.4f} | kappa={kap:.4f}")
        score = f1 + kap
        if score > best_score + 1e-6:
            best_score = score; pat_ctr = 0; ema.copy_to(model); best_state = copy.deepcopy(model.state_dict())
        else:
            pat_ctr += 1
...
def run_logo():
    print("=" * 65)
    print("  PatchHAR v2  --  ADL  --  BEST CONFIG  (N_LAYERS=8)")
    print(f"  D={cfg.D_MODEL} | H={cfg.N_HEADS} | L={cfg.N_LAYERS} | E={cfg.N_EXPERTS}")
    print("=" * 65)
    ...
    for fold_i, test_pid in enumerate(all_pids):
        ...
        val_ent   = by_pid[val_pid]; test_ent = by_pid[test_pid]

        print(f"\n{'='*65}")
        print(f"  FOLD {fold_i+1}/{len(all_pids)}  test={test_pid}  val={val_pid}")
        print(f"  train:{len(train_ent)}  val:{len(val_ent)}  test:{len(test_ent)}")
        if not val_ent or not test_ent: print("  Skip"); continue

        train_w     = sample_weights_from_entries(train_ent, num_classes)
        _, train_dl = make_loader(train_ent, pid_to_idx, cfg.BATCH_SIZE, sampler_weights=train_w, is_train=True)
        ...
        model = train_one_fold(model, train_dl, val_dl, class_w, cfg.EPOCHS, cfg.EARLY_STOP_PATIENCE)

        val_logits, val_true, _, val_seqs = collect_logits(model, val_dl)
        ts = fit_temperature(val_logits, val_true)
        best_lam, pi, A = tune_lambda(ts, val_logits, val_true, val_seqs, train_ent, num_classes)
        log_pi = np.log(np.clip(pi, cfg.HMM_MIN_PROB, 1.0)); log_A = np.log(np.clip(A, cfg.HMM_MIN_PROB, 1.0))
        ...
        kr  = cohen_kappa(test_true, pred_raw); mr = multiclass_mcc(test_true, pred_raw)
        print(f"\n  Raw   F1={f1r:.4f} | kappa={kr:.4f} | MCC={mr:.4f}")

        with torch.no_grad(): probs = ts(test_logits.to(device)).softmax(1).cpu().numpy()
        order = np.argsort(test_seqs); E_log = np.log(np.clip(probs[order], cfg.HMM_MIN_PROB, 1.0))
        ...
        kh  = cohen_kappa(test_true, pred_hmm); mh = multiclass_mcc(test_true, pred_hmm)
        print(f"  HSMM  F1={f1h:.4f} | kappa={kh:.4f} | MCC={mh:.4f}")
        print("\n" + classification_report(test_true, pred_raw,
            target_names=[idx_to_label[i] for i in range(num_classes)], zero_division=0))

        raw_scores.append((f1r, kr, mr)); hmm_scores.append((f1h, kh, mh))
    ...
    summary = {
        "raw": _summarise(raw_scores), "hsmm": _summarise(hmm_scores),
        "config": dict(dataset="ADL", D_MODEL=cfg.D_MODEL, N_HEADS=cfg.N_HEADS,
                       N_LAYERS=cfg.N_LAYERS, N_EXPERTS=cfg.N_EXPERTS,
                       n_classes=num_classes, n_participants=len(all_pids)),
    }
    print(f"\n{'='*65}\n  LOGO SUMMARY"); print(json.dumps(summary, indent=2)); print("="*65)
    return summary
