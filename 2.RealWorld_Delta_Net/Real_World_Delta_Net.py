...
def _quality_ok(w: np.ndarray, max_nan: float = 0.10,
    ...
    if sum(u <= 1 for u in unique) > 3:    return False
...
def _normalise(win: np.ndarray) -> np.ndarray:
    """Per-channel instance normalisation, clip to ±10."""
...
def load_subject_activity(sub_id: int, label: str) -> List[np.ndarray]:
    """Return list of normalised (WINDOW_SIZE, CHANNELS) windows."""
    zl = label.replace("_", "")
    ...
    # determine shared time length from readMe
    length = 1000
    ...
    # load & concatenate sensor channels -> (T, 9)
    parts = []
    for s in cfg.SENSORS:
        zf = open_zips.get(s)
        if zf is None:
            ...
            parts.append(
                _load_xyz(zf, SENSOR_PREFIX[s], zl, cfg.BODY_PART, length))
    ...
    data    = np.concatenate([p[:min_len] for p in parts], axis=1)  # (T, 9)

    # sliding windows
    stride  = max(1, int(cfg.WINDOW_SIZE * (1 - cfg.OVERLAP)))
    windows = []
    for i in range(0, len(data) - cfg.WINDOW_SIZE + 1, stride):
        w = data[i : i + cfg.WINDOW_SIZE]
        if _quality_ok(w):
            windows.append(_normalise(w))
    return windows
...
def build_all_entries(subjects: tuple) -> list:
    ...
    seq_idx is the per-subject window order used for Viterbi sorting.
    """
    ...
    for sub_id in subjects:
        ...
        for act in activities:
            ...
            for w in windows:
                entries.append((int(sub_id), w, int(ACTIVITY_LABELS[act]), seq_ctr))
                seq_ctr += 1
            done += 1
