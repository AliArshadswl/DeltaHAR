"""
realworld2016_patchhar_v2_5fold.py
====================================
PatchHAR v2 (all 10 contributions) adapted for the RealWorld2016 dataset
with five-fold subject-wise cross-validation (7:1:2 train/val/test split).
...
Model
-----
  PatchHAR v2  D_MODEL=56 | N_HEADS=4 | N_LAYERS=4 | N_EXPERTS=4
  Patch tokens only -- no statistical or topological side-features.
  C3 (circadian bias) DISABLED -- no timestamp features available.

Cross-validation
----------------
  5-fold subject-wise CV with 7:1:2 subject-level split per fold:
    n_test  = ceil(0.20 * N_subjects)   (~20%, rotated across folds)
    n_val   = ceil(0.10 * N_subjects)   (~10%, head of remainder)
    n_train = N_subjects - n_test - n_val (~70%)

Post-processing per fold
-------------------------
...
class Config:
    ...
    # Model
    D_MODEL     = 56
    N_HEADS     = 4       # per-head dim = 14 (even -> RoPE OK)
    N_LAYERS    = 4
    N_EXPERTS   = 4
    DROPOUT     = 0.30
...
def _check_window_quality(window: np.ndarray,
    ...
    constant_channels > 10 threshold matches the original code.
    """
    total = window.size
    ...
    if sum(u <= 1 for u in unique_per_ch) > 10: return False
...
class PatchHARv2(nn.Module):
    ...
    PatchHAR v2 with all 10 contributions, adapted for RealWorld2016:
      ...
      - D_MODEL = 56 | N_HEADS = 4 | N_LAYERS = 4
...
# =============================================================================
# 14.  Five-fold subject-wise CV  (7:1:2 train/val/test split)
# =============================================================================
def make_five_fold_splits(subjects: list, seed: int = 42) -> list:
    """
    Shuffle subjects and divide into 5 folds with a 7:1:2 subject-level split.

    For N subjects the block sizes are:
      n_test  = ceil(0.20 * N)   -- rotated test block (~20%)
      n_val   = ceil(0.10 * N)   -- head of the remainder (~10%)
      n_train = N - n_test - n_val  (~70%)
    ...
    n_test = max(1, math.ceil(0.20 * N))   # ~20%
    n_val  = max(1, math.ceil(0.10 * N))   # ~10%

    folds = []
    for i in range(5):
        test_start = (i * n_test) % N
        test_subs  = [subs[(test_start + j) % N] for j in range(n_test)]
        test_set   = set(test_subs)
        remain     = [s for s in subs if s not in test_set]
        val_subs   = remain[:n_val]
        train_subs = remain[n_val:]
        folds.append({"train": train_subs, "val": val_subs, "test": test_subs})
    return folds


def run_five_fold():
    print("=" * 65)
    print("  PatchHAR v2  --  RealWorld2016  --  5-Fold Subject-Wise CV (7:1:2)")
    ...
    folds = make_five_fold_splits(subjects, seed=cfg.SEED)
    ...
    for fold_i, fold in enumerate(folds):
        ...
        val_ent   = sum([by_sid[s] for s in val_sids],   [])
        test_ent  = sum([by_sid[s] for s in test_sids],  [])

        print(f"\n{'='*65}")
        print(f"  FOLD {fold_i+1}/5")
        print(f"  Train subjects ({len(train_sids)}): "
              f"{[f'proband{s}' for s in train_sids]}")
        print(f"  Val   subjects ({len(val_sids)}):   "
              f"{[f'proband{s}' for s in val_sids]}")
        print(f"  Test  subjects ({len(test_sids)}):  "
              f"{[f'proband{s}' for s in test_sids]}")
        print(f"  Windows  train:{len(train_ent)}  "
              f"val:{len(val_ent)}  test:{len(test_ent)}")

        if not train_ent or not val_ent or not test_ent:
            print("  Empty split -- skipping."); continue
