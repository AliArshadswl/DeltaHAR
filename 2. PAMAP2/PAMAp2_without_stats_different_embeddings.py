...
class Config:
    ...
    # Signal
    SIGNAL_RATE = 100       # Hz
    WINDOW_SIZE = 1000      # 10 s x 100 Hz
    ...
    PATCH_LENS_MULTI = [25, 50, 100]              # C4 multi-scale granularities
...
SEQ_CHANNELS = (
    list(sum(ACC16_TRIPLETS, ())) +
    list(sum(GYRO_TRIPLETS, ())) +
    list(sum(MAG_TRIPLETS, ())) +
    SCALARS
)
assert len(SEQ_CHANNELS) == cfg.CHANNELS, \
    f"Expected 30 channels, got {len(SEQ_CHANNELS)}"


# =============================================================================
# 5.  Data loading & windowing
# =============================================================================
...
def window_entries_for_subject(sid: int, X: np.ndarray,
                               y_str: np.ndarray,
                               class_to_idx: dict) -> list:
    """
    Slice non-overlapping windows and instance-normalise per channel.
    Entry format: (sid, window:(T, C), label_idx, seq_start)
    """
    T = X.shape[0]
    entries = []
    for start in range(0, T - cfg.WINDOW_SIZE + 1, cfg.STRIDE):
        ...
        entries.append((int(sid), win, int(class_to_idx[maj]), int(start)))
    return entries
...
def build_all_entries(df: pd.DataFrame):
    ...
    for sid, df_s in df.groupby(cfg.SUBJECT_COL, sort=True):
        ...
        entries.extend(
            window_entries_for_subject(int(sid), X, y, class_to_idx))
    return entries, labels, class_to_idx, idx_to_class


# =============================================================================
# 6.  [C5] Frequency-aware data augmentation
# =============================================================================
...
class PAMAP2Dataset(Dataset):
    ...
    Per-window output:
      ...
      label         long scalar
      sid           long scalar
      seq_start     long scalar  (window start index, used for Viterbi ordering)
...
# =============================================================================
# 8.  Training utilities: WarmupCosine, EMA, class/sample weights
# =============================================================================
class WarmupCosine:
...
class DualDomainPatchEmbed(nn.Module):
    """
    [C1] Projects each patch in both time and frequency domains, fused with a
    learnable per-output-dim gate:  out = sigmoid(g) * time + (1-sigmoid(g)) * freq
    """
...
class ReconHead(nn.Module):
    ...
    Output dim = N_PATCHES x CHANNELS = 40 x 30 = 1200.
    """
...
# =============================================================================
# 10.  PatchHAR v2  -  PAMAP2 edition
# =============================================================================
class PatchHARv2(nn.Module):
    """
    PatchHAR v2 with all 10 contributions adapted for PAMAP2:
      - CHANNELS = 30
      - C3 circadian bias DISABLED (no timestamps used as features)
      - Patch tokens only: no statistical or topological side-features
      - N_PATCHES = 40  (1000 samples / 25 sample primary patch)
    """
    def __init__(self, num_classes: int):
        ...
        # RoPE frequency cache sized to N_PATCHES tokens
...
# =============================================================================
# 12.  Temperature scaling & HSMM-lite
# =============================================================================
...
def train_one_fold(model: PatchHARv2,
    ...
    for epoch in range(epochs):
        ...
        print(f"    Ep {epoch+1:02d}/{epochs} | F1={f1:.4f} | kappa={kap:.4f}")
...
def run_logo():
    ...
    print("Building windowed entries ...")
