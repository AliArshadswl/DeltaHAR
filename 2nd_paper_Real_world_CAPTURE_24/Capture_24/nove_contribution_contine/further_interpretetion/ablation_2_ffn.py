"""
Ablation 2: Soft-MoE → standard FFN (all other components KEPT).

The two SoftMoE blocks (moe1 and moe2) are replaced with a plain
two-layer feed-forward network: Linear → SiLU → Dropout → Linear.
Hidden dim is kept the same (2 * D_MODEL) so the comparison is fair.

Everything else — multiscale patching, circadian bias, stochastic depth,
prototype memory, freq augmentation, label smoothing, temperature scaling,
manifold mixup, recon aux loss — is ACTIVE.
"""
...
class ContribConfig:
    C1_DUAL_DOMAIN_EMBEDDING   = True   # keep off (same as original default)
    ...
    # MoE is ablated structurally — no flag needed

CC = ContribConfig()
print("  [Ablation 2] Soft-MoE → FFN  (all other components kept)")


class Config:
    PROC_DIR   = Path("/mnt/share/ali/processed/")
    OUTPUT_DIR = Path("/mnt/share/ali/processed/patchhar_results/ablation2_ffn/")
...
def freq_augment(sig):
    fn = random.choice([_bandpass_jitter, _axis_permute, _magnitude_scale, _time_warp])
    return fn(sig)
...
# ── KEY CHANGE: FFN replaces SoftMoE ─────────────────────────────────────────
class FFN(nn.Module):
    """Standard 2-layer FFN, drop-in replacement for SoftMoE.
    Hidden dim = 2*d to match the expert hidden size in SoftMoE.
    """
...
class HierarchicalPatchEmbed(nn.Module):
    ...
    def forward(self, patches_list):
        ...
        e_mid          = self.embed_mid(p_mid)
        e_coarse       = self.embed_coarse(p_coarse)
        ...
        return e_fine + e_mid_broad + e_coarse_broad
...
class PatchHARv2(nn.Module):
    """Same as original but moe1 and moe2 are FFN blocks."""
    def __init__(self):
        super().__init__()
        d  = cfg.D_MODEL
        NP = cfg.N_PATCHES
        ...
        # ── ABLATION: FFN in place of SoftMoE ────────────────────────────────
        self.moe1 = FFN(d, 2*d, dropout=cfg.DROPOUT)
        self.attn = GatedAttention(d, n_heads=cfg.N_HEADS, dropout=cfg.DROPOUT)
        self.moe2 = FFN(d, 2*d, dropout=cfg.DROPOUT)
        # ─────────────────────────────────────────────────────────────────────
...
def recon_loss(recon, raw_segs):
    ...
    target  = (raw_segs[:, :NP*PL, :]
               .reshape(B, NP, PL, C).permute(0, 1, 3, 2)
               .reshape(B, NP * C * PL))
...
def main():
    print("=" * 70)
    print("  PatchHAR v2 — Ablation 2: Soft-MoE → FFN")
    print("=" * 70)
    ...
    save_path = cfg.OUTPUT_DIR / "weights_ablation2.pth"
    ...
    results = {
        "ablation":    "2_softmoe_to_ffn",
        "description": "SoftMoE replaced with plain FFN, all other components kept",
        "metrics":     metrics,
        "training":    history,
        "config": {
    ...
    out = cfg.OUTPUT_DIR / "results_ablation2.json"
