"""
Ablation 3: GatedDeltaNet → standard self-attention (all other components KEPT).

The N_LAYERS GatedDeltaNet blocks are replaced with standard
nn.MultiheadAttention + pre-norm + residual connections.  The MoE blocks,
GatedAttention mixer, stochastic depth, circadian bias, multiscale patching,
freq augmentation, label smoothing, prototype memory, manifold mixup, and
recon aux loss are all still ACTIVE — only the recurrent-style GDN backbone
is swapped out.

Fair comparison notes:
  • Same n_heads (N_HEADS=2) as the GatedAttention mixer.
  • Same dropout (DROPOUT=0.25).
  • Pre-norm via ZCRMSNorm (same as GDN) for a like-for-like comparison.
  • Stochastic depth wrapping is preserved when C8 is enabled.
"""
from __future__ import annotations
import math, random, json, warnings
...
CC = ContribConfig()
print("  [Ablation 3] GDN → self-attention  (all other components kept)")


class Config:
    PROC_DIR   = Path("/mnt/share/ali/processed/")
    OUTPUT_DIR = Path("/mnt/share/ali/processed/patchhar_results/ablation3_selfattn/")
    ...
    def _update(self, actual_window):
...
def _time_warp(sig):
    ...
    for c in range(C):
        warped[:, c] = np.interp(np.linspace(0, T-1, new_T), np.arange(T), sig[:, c])
    if new_T >= T: return warped[:T]
    out = np.zeros((T, C), dtype=sig.dtype); out[:new_T] = warped
    return out
...
class WindowDataset(Dataset):
    def __init__(self, pid_list, proc_dir, class_to_idx, is_train=False):
        ...
        for pi, pid in enumerate(pid_list):
            ...
            if not _set:
                if W.shape[1] != cfg.WINDOW_SIZE: cfg._update(W.shape[1])
                _set = True
            for w, lab, f in zip(W, L, F):
                if lab not in class_to_idx: continue
    ...
    def _make_patches(seg, patch_len):
        T, C = seg.shape; n_p = T // patch_len; seg = seg[:n_p * patch_len]
    ...
    def __getitem__(self, idx):
        ...
        return (patches_list, torch.from_numpy(tfeat),
                torch.tensor(label, dtype=torch.long), pid,
...
# ── KEY CHANGE: SelfAttentionBlock replaces GatedDeltaNet ─────────────────────
class SelfAttentionBlock(nn.Module):
    """Standard multi-head self-attention with pre-norm and residual.
    Intentionally vanilla — no RoPE, no gate — to isolate the GDN contribution.
    Uses the same ZCRMSNorm as the rest of the model.
    """
...
def precompute_freqs(dim, n_tok, theta=10000.0):
    ...
    return torch.polar(torch.ones(n_tok, dim // 2), torch.outer(torch.arange(n_tok), freqs))
...
class GatedAttention(nn.Module):
    """The *mixer* GatedAttention is kept; only the backbone layers change."""
    def __init__(self, d, n_heads=2, dropout=0.1):
        super().__init__()
...
class HierarchicalPatchEmbed(nn.Module):
    def __init__(self, patch_lens, channels, d):
        super().__init__()
        self.patch_lens    = sorted(patch_lens)
        fine_pl, mid_pl, coarse_pl = self.patch_lens
        self.repeat_mid    = mid_pl    // fine_pl
        self.repeat_coarse = coarse_pl // fine_pl
        self.embed_fine    = SimplePatchEmbed(fine_pl,   channels, d)
        self.embed_mid     = SimplePatchEmbed(mid_pl,    channels, d)
        self.embed_coarse  = SimplePatchEmbed(coarse_pl, channels, d)

    def forward(self, patches_list):
        p_fine, p_mid, p_coarse = patches_list
        e_fine   = self.embed_fine(p_fine)
        e_mid    = self.embed_mid(p_mid).repeat_interleave(self.repeat_mid,    dim=1)
        e_coarse = self.embed_coarse(p_coarse).repeat_interleave(self.repeat_coarse, dim=1)
        return e_fine + e_mid + e_coarse


class CircadianBias(nn.Module):
    def __init__(self, n_patches, d):
        super().__init__()
        self.mlp = nn.Sequential(nn.Linear(5, d), nn.SiLU(),
...
class PatchHARv2(nn.Module):
    def __init__(self):
        ...
        if CC.C4_MULTISCALE_PATCHING:
            self.hier_embed = HierarchicalPatchEmbed(cfg.PATCH_LENS_MULTI, cfg.CHANNELS, d)
        else:
        ...
        # ── ABLATION: SelfAttentionBlock instead of GatedDeltaNet ─────────────
        raw_layers = [SelfAttentionBlock(d, n_heads=cfg.N_HEADS, dropout=cfg.DROPOUT)
...
def compute_class_weights(ds):
    ...
    for e in ds.entries: counts[e[3]] += 1
...
def manifold_mixup(x, labels, alpha=0.2):
    if alpha <= 0: return x, labels, labels, 1.0
...
def raw_mixup(patches_list, times, labels, alpha=0.2):
    if alpha <= 0: return patches_list, times, labels, labels, 1.0
    lam = float(np.random.beta(alpha, alpha))
...
def recon_loss(recon, raw_segs):
    B, T, C = raw_segs.shape; NP = cfg.N_PATCHES; PL = cfg.PATCH_LEN
    target = (raw_segs[:, :NP*PL, :].reshape(B, NP, PL, C)
              .permute(0, 1, 3, 2).reshape(B, NP * C * PL))
...
def train(model, train_dl, val_dl, class_w, save_path):
    if save_path.exists():
        try:
            ...
            if set(ckpt.keys()) != set(model.state_dict().keys()): save_path.unlink()
        except Exception: save_path.unlink()
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        for batch in train_dl:
            ...
            if CC.C9_MANIFOLD_MIXUP:
                with amp_ctx():
                    z = model._embed_patches(patches_list)
                    if CC.C3_CIRCADIAN_BIAS: z = z + model.circ_bias(times)
                    else: z = z + model.time_emb(times).unsqueeze(1)
                    z = model.input_norm(z)
                    for layer in model.delta_layers: z = layer(z)
                    z = z + model.moe1(z)
                    z = model.attn(z, model.freqs)
                    z = z + model.moe2(z)
                    z_pool = z.mean(1)

                z_mix, la, lb, lam = manifold_mixup(z_pool, labels, cfg.MIXUP_ALPHA)
                ...
                with amp_ctx():
                    ...
                    if CC.C10_RECON_AUX_GRAD_SURGERY:
                        rl   = recon_loss(model.recon_head(z_pool.detach()), raw_segs)
        ...
        with torch.no_grad():
            for batch in val_dl:
                ...
                if CC.C7_PROTOTYPE_MEMORY:
                    z         = model.forward(patches_list, times_d, return_embedding=True)
                    logits, _ = model(patches_list, times_d)
                    embs_val.append(z.cpu()); labs_val.append(labels)
                else:
                    logits, _ = model(patches_list, times_d)
        ...
        if CC.C7_PROTOTYPE_MEMORY and embs_val:
            model.update_prototypes(torch.cat(embs_val).to(device),
...
def main():
    print("=" * 70)
    print("  PatchHAR v2 — Ablation 3: GDN → Standard Self-Attention")
    print("=" * 70)
    ...
    save_path = cfg.OUTPUT_DIR / "weights_ablation3.pth"
    ...
    results = {
        "ablation":    "3_gdn_to_selfattn",
        "description": "GatedDeltaNet replaced with standard self-attention; all other components kept",
        "metrics":     metrics,
        "training":    history,
        "config": {
            "window_size": cfg.WINDOW_SIZE, "patch_len": cfg.PATCH_LEN,
            "d_model": cfg.D_MODEL, "n_heads": cfg.N_HEADS,
            "n_layers": cfg.N_LAYERS, "n_params": n_params, "device": str(device),
        },
    }
    out = cfg.OUTPUT_DIR / "results_ablation3.json"
