        ...
        for pi, pid in enumerate(pid_list):
            ...
            if not _set:
                if W.shape[1] != cfg.WINDOW_SIZE:
                    print(f"  [INFO] Window size {cfg.WINDOW_SIZE} → {W.shape[1]}")
...
class PatchHARv2(nn.Module):
    def __init__(self):
        ...
        if CC.C4_MULTISCALE_PATCHING:
            ...
            self.single_embed = _make_patch_embedder(cfg.PATCH_LEN, cfg.CHANNELS, d)
    ...
    def _embed_patches(self, patches_list: list[torch.Tensor]) -> torch.Tensor:
        if CC.C4_MULTISCALE_PATCHING:
...
def manifold_mixup(x: torch.Tensor, labels: torch.Tensor, alpha: float = 0.2):
    if alpha <= 0:
...
def recon_loss(recon: torch.Tensor, raw_segs: torch.Tensor) -> torch.Tensor:
...
class SmoothCE(nn.Module):
    def __init__(self, weight: torch.Tensor | None = None, smoothing: float = 0.0):
        super().__init__()
...
def train(model: PatchHARv2,
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if score > best_score + 1e-6:
            ...
            torch.save(model.state_dict(), save_path)
            try:
                _verify     = torch.load(save_path, map_location="cpu", weights_only=False)
