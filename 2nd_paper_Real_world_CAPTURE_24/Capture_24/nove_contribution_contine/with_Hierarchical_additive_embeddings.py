    ...
    if save_path.exists():
        try:
            ...
            if any("delta1." in k for k in ckpt.keys()):
                ...
                # also check for old interpolation-based C4 key
                if any("scale_fusion" in k for k in ckpt.keys()):
                    print(f"  [Fix] Old interpolation-based C4 checkpoint "
                          f"detected. Deleting: {save_path}")
                    save_path.unlink()
        except Exception:
            save_path.unlink()
