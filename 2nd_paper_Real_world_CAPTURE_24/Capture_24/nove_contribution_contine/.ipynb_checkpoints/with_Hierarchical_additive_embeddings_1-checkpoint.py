...
def train(model: PatchHARv2,
    ...
    if save_path.exists():
        try:
            ...
            # This should never happen now that detection compares full key-sets,
            # but guard anyway so a 30-epoch run is never wasted.
            print(f"  [Warn] Could not load best checkpoint — key mismatch: {e}")
            print(f"  [Info] Keeping in-memory model weights (last trained epoch).")
    else:
        print(f"  [Warn] No checkpoint found at {save_path}. Keeping in-memory weights.")
    return history
