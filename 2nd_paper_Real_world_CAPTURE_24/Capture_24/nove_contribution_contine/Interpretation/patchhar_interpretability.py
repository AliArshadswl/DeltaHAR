...
def plot_attention_rollout(
    ...
    fig, ax = plt.subplots(figsize=(max(12, NP // 8), max(4, K * 0.55)))
    im = ax.imshow(mat_n, aspect="auto", cmap=CMAP_SEQ, vmin=0, vmax=1)
    ax.set_yticks(range(K))
    ax.set_yticklabels(CLASSES, fontsize=8)
    ax.set_xticks(range(0, NP, step))
    ax.set_xticklabels([f"{t:.1f}s" for t in patch_sec[::step]], fontsize=7, rotation=45)
...
def plot_moe_analysis(outputs: dict):
    ...
    for tag, moe_w in [("MoE1", outputs["moe1_w"]),
        ...
        fig, ax = plt.subplots(figsize=(max(5, E), max(4, K * 0.55)))
    ...
    if outputs["moe1_w"] is not None:
        ...
        plt.colorbar(sc0, ax=axes[0], ticks=range(K),
                     label="Class").ax.set_yticklabels(CLASSES, fontsize=6)
        ...
        plt.colorbar(sc1, ax=axes[1], ticks=range(E),
                     label="Expert").ax.set_yticklabels(
                         [f"E{i}" for i in range(E)], fontsize=8)

        plt.suptitle("MoE Expert Specialisation in Embedding Space", fontsize=12)
        plt.tight_layout()
...
def plot_calibration(outputs: dict):
    ...
    fig, ax = plt.subplots(figsize=(max(8, len(cls_names) * 0.7), 4))
...
def plot_gate_analysis(model: PatchHARv2, outputs: dict):
    ...
    if gate_samples:
        ...
        fig, ax = plt.subplots(figsize=(max(8, NUM_CLASSES * 0.7), 4))
...
def plot_prototype_analysis(model: PatchHARv2, outputs: dict):
    ...
    fig, ax = plt.subplots(figsize=(max(6, K * 0.6), max(5, K * 0.55)))
...
def plot_confusion_matrix(outputs: dict):
    ...
    fig, ax = plt.subplots(figsize=(max(8, NUM_CLASSES * 0.75),
                                    max(6, NUM_CLASSES * 0.65)))
    sns.heatmap(cm_norm, annot=True, fmt=".2f",
                xticklabels=CLASSES, yticklabels=CLASSES,
                cmap="Blues", linewidths=0.4, ax=ax, annot_kws={"size": 7})
...
def plot_circadian_errors(outputs: dict):
    ...
    fig, axes = plt.subplots(1, len(worst_k),
                             figsize=(5 * len(worst_k), 4), sharey=False)
    if len(worst_k) == 1:
        axes = [axes]
    ...
    plt.suptitle("Circadian Error Analysis — Worst Performing Classes", fontsize=12)
    plt.tight_layout()
...
def plot_signal_difficulty(outputs: dict):
    ...
    for k, (x, y) in enumerate(zip(cls_diff, recall)):
        axes[1].annotate(CLASSES[k], (x, y), fontsize=6,
    ...
    plt.suptitle("Signal Difficulty Analysis", fontsize=12)
    plt.tight_layout()
...
def plot_gradcam_worst(
    ...
    fig, axes = plt.subplots(n_pairs, n_per_pair,
                             figsize=(5 * n_per_pair, 3 * n_pairs))
    if n_pairs == 1:
        axes = axes[None]
    if n_per_pair == 1:
        axes = axes[:, None]
...
def plot_reconstruction_analysis(
    ...
    fig, axes    = plt.subplots(len(show_classes), cfg.CHANNELS,
                                figsize=(5 * cfg.CHANNELS, 3 * len(show_classes)))
    ...
    plt.suptitle("Raw vs Reconstructed Accelerometer Signal  (C10)", fontsize=12)
    plt.tight_layout()
...
def train_with_gradient_conflict_logging(
    ...
    plt.suptitle("PCGrad Gradient Surgery Analysis  (C10)", fontsize=12)
    plt.tight_layout()
...
def plot_multiscale_ablation(model: PatchHARv2, loader: DataLoader):
    ...
    w    = 0.22
    fig, axes = plt.subplots(2, 1, figsize=(max(12, NUM_CLASSES * 0.85), 9))
