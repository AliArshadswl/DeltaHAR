...
C4  Adaptive patch length via multi-scale patching — the window is split at
    THREE granularities (25, 50, 100 samples) and the resulting token
    sequences are processed in parallel then mean-fused before the transformer
    stack.  Lets the model attend to sub-second gestures AND multi-second
...
─────────────────────────────────────────
  ...
  ↓
  [C4] Multi-scale patching  ×3 granularities → fused (120 × 64)
...
class ContribConfig:
    ...
    C4_MULTISCALE_PATCHING     = True   # 3 patch granularities
...
class Config:
    ...
    PATCH_LEN   = 25          # samples per patch  (primary granularity)
    CHANNELS    = 3           # x, y, z axes
    N_PATCHES   = WINDOW_SIZE // PATCH_LEN   # 120

    # [C4] secondary and tertiary granularities
    PATCH_LENS_MULTI = [25, 50, 100]   # only used when C4 is ON
...
class DualDomainPatchEmbed(nn.Module):
    """
    Projects each patch in both time and frequency domains, fuses with a
    learnable gate:
...
class PatchHARv2(nn.Module):
    ...
    def __init__(self):
        ...
        # ── Patch embedding (one per granularity for C4) ──────────────────
        self.patch_lens = (cfg.PATCH_LENS_MULTI if CC.C4_MULTISCALE_PATCHING
                           else [cfg.PATCH_LEN])
        if CC.C1_DUAL_DOMAIN_EMBEDDING:
            self.patch_embeds = nn.ModuleList([
                DualDomainPatchEmbed(pl, cfg.CHANNELS, d)
                for pl in self.patch_lens
            ])
        else:
            self.patch_embeds = nn.ModuleList([
                SimplePatchEmbed(pl, cfg.CHANNELS, d)
                for pl in self.patch_lens
            ])

        # [C4] scale fusion: project concatenated multi-scale tokens → NP×D
        if CC.C4_MULTISCALE_PATCHING and len(self.patch_lens) > 1:
            # each scale gives (B, NP_i, D);  we up/down-sample to NP via
            # adaptive avg pool then average across scales
    ...
    def _embed_patches(self,
        ...
        if CC.C4_MULTISCALE_PATCHING and len(patches_list) > 1:
            ...
            for embed, patches in zip(self.patch_embeds, patches_list):
                ...
                # interpolate to canonical NP
                e = e.permute(0, 2, 1)          # (B, D, N_Pi)
                e = F.interpolate(e, size=NP,
                                  mode="linear",
                                  align_corners=False)
                e = e.permute(0, 2, 1)          # (B, NP, D)
                emb_list.append(e)
            x = torch.cat(emb_list, dim=-1)    # (B, NP, D*n_scales)
            x = self.scale_fusion(x)            # (B, NP, D)
        else:
...
def train(model: PatchHARv2,
    ...
    # ── BUG FIX: delete stale checkpoint that has incompatible key names ──
    if save_path.exists():
        try:
            ckpt = torch.load(save_path, map_location="cpu",
                              weights_only=False)
            # check a key that changed between v1 and v2
            if any("delta1." in k for k in ckpt.keys()):
                print(f"  [Fix] Stale checkpoint detected "
                      f"(v1 key names). Deleting: {save_path}")
                save_path.unlink()
            else:
                # also check shape mismatch on a sentinel key
                if ("patch_proj.weight" in ckpt and
                        ckpt["patch_proj.weight"].shape !=
                        model.patch_embeds[0].state_dict()[
                            list(model.patch_embeds[0].state_dict())[0]
                        ].shape):
                    print(f"  [Fix] Shape mismatch in checkpoint. "
        ...
        with torch.no_grad():
            for batch in val_dl:
                ...
                if CC.C7_PROTOTYPE_MEMORY:
                    ...
                    # z is embedding only when return_embedding=True
                    # we call again for logits — or restructure forward:
    ...
    # ── Load best checkpoint (now guaranteed to be compatible) ───────────
